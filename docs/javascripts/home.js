/* ==========================================================================
   ...
   - drifting ink-leaf field on a canvas behind the hero
...
(function () {
  ...
  /* ---- Drifting ink-leaf field --------------------------------------- */
  // Leaves, not coins: coins are off-brand, and the app already uses falling
  // ink leaves for its own celebration confetti, so this matches the product.
  function initPetals() {
    var canvas = document.getElementById("bs-petals");
    if (!canvas || reduce) return;
    var ctx = canvas.getContext("2d");
    var petals = [];
    var raf = null;
    var dpr = Math.min(window.devicePixelRatio || 1, 2);
    // Brand palette only: espresso and earth ink, terracotta, sand.
    var colors = ["#5A4A3C", "#B07C5E", "#D9C6A8", "#262219", "#C4A98B"];
    ...
    // Sparser than the old coin field: a few leaves read as calm, a swarm does not.
    var count = Math.max(12, Math.min(30, Math.floor(rect.width / 46)));
    ...
    // An almond leaf: two mirrored quadratic curves from base to tip. The same
    // shape the app draws for its sprig motif and confetti.
    function leafPath(r) {
      var half = r * 1.5;
      var bulge = r * 0.52;
      ctx.beginPath();
      ctx.moveTo(0, -half);
      ctx.quadraticCurveTo(bulge, 0, 0, half);
      ctx.quadraticCurveTo(-bulge, 0, 0, -half);
      ctx.closePath();
    }
    ...
    function draw() {
      ...
      for (var i = 0; i < petals.length; i++) {
        ...
        // The sway drives the tilt too, so a leaf turns as it drifts rather
        // than spinning on the spot.
        ctx.rotate(c.a + Math.sin(c.swayA) * 0.5);
        ctx.globalAlpha = c.o;
        ctx.fillStyle = c.c;
        leafPath(c.r);
        ctx.fill();
        // A centre vein, drawn at the same alpha so it stays a hint.
        ctx.strokeStyle = c.c;
        ctx.lineWidth = Math.max(0.5, c.r * 0.09);
        ctx.beginPath();
        ctx.moveTo(0, -c.r * 1.4);

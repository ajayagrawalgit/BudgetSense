...
> Your financial data leaves your phone only if you deliberately export a file
> yourself, or you deliberately turn on the optional Google Drive backup, and in
> that case it is encrypted on your device before it is uploaded.

**Effective for:** BudgetSense 0.1
...
The Android app declares the `INTERNET` permission for one feature: the optional
Google Drive backup. It sends no financial data anywhere the developer can read
it, and it makes no network request at all until you explicitly enable it and
complete setup, which is enforced in code rather than only by policy.
...
One thing worth being clear about: this database is not separately encrypted by
the app. It relies on the protection Android gives every app, meaning the
private sandbox plus your device's own disk encryption and lock screen. On a
normal, locked device another app cannot read it. On a rooted device, or one
left unlocked, treat it as readable. Turning on the app lock and keeping a
device passcode are the two things that help most.
...
- **Internet and network state:** for the optional Google Drive backup only. No
  financial data leaves your device unless you turn cloud backup on.
...
- **Storage / file access:** used only when you choose to export a local backup
  or import one that you selected. The app goes through the system file picker,
  so you pick the exact file or folder and nothing else becomes readable.
...
Released builds of the app write no diagnostic log at all. Logging is compiled
out of anything but a debug build (see `lib/core/utils/app_log.dart`), so the
version you install cannot leave error detail behind in the system log.
